...
let rec type_to_sort t =
  if Types.is_bool_type t then bool_sort
    ...
    | Types.Tstatic (_, ty) ->
      type_to_sort ty
    | Types.Tarrow _ | _ ->
      Format.eprintf "internal error: pp_type %a@." Types.pp t;
      assert false
...
let pp_fdecls fmt =
  Format.fprintf
    fmt
    "Registered fdecls: @[%a@]@ "
    (Utils.Format.pp_print_list Format.pp_print_string)
...
let decl_rel ?(no_additional_vars = false) name args_sorts =
  ...
  if !debug then
    Format.eprintf
      ...
      name
      (Utils.Format.pp_print_list (fun fmt sort ->
...
let rec instr_to_exprs machines reset_instances (m : machine_t) instr :
    Z3.Expr.expr list * Utils.ident list =
