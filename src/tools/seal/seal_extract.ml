        ...
        if Z3.Expr.is_numeral ze then
          let e =
            if Z3.Arithmetic.is_real ze then
              ...
              mkexpr Location.dummy (Expr_const (Const_real real))
              ...
              | "true" ->
                mkexpr Location.dummy (Expr_const (Const_tag tag_true))
              | "false" ->
                mkexpr Location.dummy (Expr_const (Const_tag tag_false))
              | _ ->
                assert false
            else (
              Format.eprintf
          ...
          | "not" | "=" | "-" | "*" | "/" | ">=" | "<=" | ">" | "<" ->
            ...
            None, Some (mkpredef_call Location.dummy op args)
          | "+" ->
            ...
            let e =
              ...
              | e1 :: e2 :: tl ->
                let first_binary_and =
                  mkpredef_call Location.dummy op [ e1; e2 ]
                in
                if tl = [] then first_binary_and
                else
                  List.fold_left
                    (fun e e_new ->
                      mkpredef_call Location.dummy op [ e; e_new ])
                    first_binary_and
                    tl
            in
                ...
                (fun (is_init_opt1, expr_opt1) (is_init_opt2, expr_opt2) ->
                  ( (match is_init_opt1, is_init_opt2 with
                    ...
                    | None, x | x, None ->
                      x
                    | Some e1, Some e2 ->
                      Some (mkpredef_call Location.dummy op [ e1; e2 ]) ))
                hd
                tl)
          | op ->
...
let check_sat ?(just_check = false) (l : elem_boolexpr guard) :
  ...
  try
    ...
    if false then
      Format.eprintf
        "Z3 exprs1: [%a]@ "
        (Format.pp_print_list ~pp_sep:Format.pp_print_comma (fun fmt e ->
...
and add_def defs vid expr =
  ...
  report ~level:6 (fun fmt ->
      Format.fprintf
        ...
        Printers.pp_expr
        expr
        (Format.pp_print_list (pp_guard_expr pp_elem))
        vid_defs);
          ...
          List.map
            (fun (m, gel) ->
              ...
              | [ (_, e) ] ->
                ...
                m, e
              | [] ->
                m, Corelang.expr_of_ident m Location.dummy
              | _ ->
                assert false)
            mem_defs );
      ]
...
let build_environement consts (mems : var_decl list) nd =
  ...
  report ~level:3 (fun fmt ->
      Format.fprintf
        fmt
        "@[<v 0>%a@]@."
        (Format.pp_print_list (fun fmt (m, mdefs) ->
             Format.fprintf
               fmt
               "%s -> [@[<v 0>%a@] ]@ "
               m
